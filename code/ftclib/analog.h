...
// Constants.
//
#define ANALOGF_USER_MASK       0x00ff
#define ANALOGF_HTSMUX          0x0001
#define ANALOGF_INVERSE         0x0002
    ...
    )
{
    ...
    if ((numSamples != 0) && (calInterval != 0))
    {
        ...
        for (i = 0; i < numSamples; i++)
        {
            ...
            data = (analog.analogFlags & ANALOGF_HTSMUX)?
 ...
 *  offset and deadband.
 *
 *  @param analog Points to the ANALOG structure to be initialized.
 */
int
AnalogReadValue(
    __inout ANALOG &analog,
    )
{
    int data;
    ...
    if (analog.analogFlags & ANALOGF_INVERSE)
    {
        data = -data;
    }
 ...
 *  @param analogFlags Specifies the analog flags.
 *  @param numSamples Specifies the number of calibration samples.
 *  @param calInterval Specifies the calibration interval in msec.
 */
void
AnalogInit(
    ...
    __in  int analogFlags = 0,
    __in  int numSamples = 0,
    __in  int calInterval = 0
    )
{
    ...
    analog.analogFlags = analogFlags & ANALOGF_USER_MASK;
